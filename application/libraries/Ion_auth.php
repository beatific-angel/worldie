...
class Ion_auth
{
	...
	public function __construct()
	{
		...
		$this->_cache_user_in_role =& $this->ion_auth_model->_cache_user_in_role;
	 ...
	 * @return bool
	 */
	public function register($identity, $password, $email, $additional_data = array(), $role_ids = array()) //need to test email activation
	{
	 ...
	 * @author jrmadsen67
	 **/
	public function has_permission($action=null)
	{	
		$user_roles  = $this->config->item('user_roles', 'user_permission');
		$permissions = $this->config->item('user_roles_permissions', 'user_permission');
	 ...
	 * @author jrmadsen67
	 **/
	public function has_group_permission($action=null)
	{	
		/* $user_roles  = $this->config->item('user_roles', 'user_permission');
		$permissions = $this->config->item('group_user_roles_permissions', 'user_permission'); 
		$user_roleid = $this->session->userdata('role_id'); */
		$permissions = $this->config->item('group_user_roles_permissions', 'user_permission'); 
	...
	public function is_admin($id=false)
	{
		...
		$admin_role = $this->config->item('admin_role', 'ion_auth');

		return $this->in_role($admin_role, $id);
	}

	/**
	 * in_role
	 *
	 * @param mixed role(s) to check
	 * @param bool user id
	 * @param bool check if all roles is present, or any of the roles
	 *
	 * @return bool
	 * @author Phil Sturgeon
	 **/
	public function in_role($check_role, $id=false, $check_all = false)
	{
		...
		if (isset($this->_cache_user_in_role[$id]))
		{
			$roles_array = $this->_cache_user_in_role[$id];
		}
		else
		{
			$users_roles = $this->ion_auth_model->get_users_roles($id)->result();
			$roles_array = array();
			foreach ($users_roles as $role)
			{
				$roles_array[$role->id] = $role->name;
			}
			$this->_cache_user_in_role[$id] = $roles_array;
		}
