...
class Post_model extends CI_Model {
    ...
    public function deleteGroupPostContent($id, $reported_post = true) {
        ...
        if($get_record->album_id>0){
             ...
             if(!empty($get_album)){
                ...
                #delete album itmes
                $get_album_itmes = $this->db->query('SELECT * FROM group_album_items WHERE album_id='.$get_album->id)->result();
    ...
    public function deletePagePostContent($id, $reported_post = true) {
        $user_id = $this->session->userdata('user_id');
        if ($reported_post) {
            $results = $this->db->query('SELECT * FROM reported_content WHERE id=' . $id)->row();
            $post_id = $results->report_id;
        ...
        //delete page post comment (1)
        $c_records = $this->db->query('SELECT * FROM page_post_comment WHERE post_id=' . $post_id)->result();
        if (count($c_records) > 0) {
            foreach ($c_records as $record) {
                $this->db->where('id', $record->id);
                $this->db->delete('page_post_comment');
            }
        }

        //delete page post view (2)
        $v_records = $this->db->query('SELECT * FROM page_post_views WHERE post_id=' . $post_id)->result();
        if (count($v_records) > 0) {
            foreach ($v_records as $record) {
                $this->db->where('id', $record->id);
                $this->db->delete('page_post_views');
            }
        }

        //delete page post like (3)
        $l_records = $this->db->query('SELECT * FROM page_post_like WHERE post_id=' . $post_id)->result();
        if (count($l_records) > 0) {
            foreach ($l_records as $record) {
                $this->db->where('id', $record->id);
                $this->db->delete('page_post_like');
            }
        }
        
        //delete group post wall share (4)
        $w_records = $this->db->query('SELECT * FROM wall_shared_post WHERE post_id=' . $post_id . ' AND content_type=15')->result();
        if (count($w_records) > 0) {
            foreach ($w_records as $record) {
                $this->db->where('id', $record->id);
                $this->db->delete('wall_shared_post');
        ...
        //delete page post (5)
        $get_record = $this->db->query('SELECT * FROM page_posts WHERE id='.$post_id)->row();
        if(!empty($get_record)){
          #get image & delete with subcomponent(6)
          if($get_record->image_id>0){
             $this->common->delete_image_with_component($get_record->image_id,'assets/page/post_image/');
          }
          ...
          if($get_record->video_id>0){
             $get_video = $this->db->query('SELECT * FROM video WHERE id='.$get_record->video_id)->row();
             if(!empty($get_video)){
                $vid_name = 'assets/page/post_video/'.$get_image->prefix.'video_'.$get_video->id.'.'.strtolower($get_video->type);
          ...
          #get album & delete
        if($get_record->album_id>0){
             $get_album = $this->db->query('SELECT * FROM page_albums WHERE id='.$get_record->album_id)->row();
             if(!empty($get_album)){

                $this->db->where('id', $get_album->id);
                $this->db->delete('page_albums');

                #delete album itmes
                $get_album_itmes = $this->db->query('SELECT * FROM page_album_items WHERE album_id='.$get_album->id)->result();
                if(count($get_album_itmes)>0){
                    foreach ($get_album_itmes as $record) {
                       $this->db->where('id', $record->id);
                       $this->db->delete('page_album_items');
    ...
    private function _post_filter_list($param = NULL, $pstatus, $userid) {
        $sql = $sql_custom = array();
        $f_sql = $f_sql_custom = $custom_where = '';
        foreach ($this->column_search as $cmn) {
            if (isset($_POST["search"]["value"]) && $_POST["search"]["value"] != '') {
        ...
        $this->db->select("a.id AS data_item_id, a.user_id AS data_userid, a.content AS data_content, a.show_status AS data_privacy, a.share_count AS data_sharecount, a.sponsered AS data_sponsered, a.created_at AS data_created_at, a.updated_at AS data_updated_at, a.image_id AS data_image, a.video_id AS data_video, a.post_type AS data_item_type, a.album_id AS data_albumid, b.first_name AS data_f_name, b.last_name AS data_l_name, b.gender AS data_gender,b.ip_address AS ip_address, c.image AS data_userimage, c.thumb_image AS data_thumb_image,a.status as status, b.country, b.state, b.city, b.address, a.blocked_status, a.blocked_way, a.block_text_id");
        $this->db->from("post a");
        $this->db->join('users b', 'b.id = a.user_id');
        $this->db->join('users_profile c', 'b.id = c.user_id');
        //$this->db->join('blocked_text d', 'd.id = a.block_text_id');
        if (!empty($custom_where)) {
            $this->db->where($custom_where);
        }
        if ($pstatus != -1){
            $this->db->where('blocked_status', $pstatus);
        }
        if ($userid != -1){
    ...
    public function postFilterList($show_list, $pstatus, $userid = -1){
        $query = $this->_post_filter_list($show_list, $pstatus, $userid);
        $result = $this->db->query($query)->result();

        $new_result = array();
        foreach($result as $record) {
            if($record->blocked_status == 1 && $record->blocked_way == 1){
                $this->db->select('*');
                $this->db->from('blocked_text');
                $this->db->where('id', $record->block_text_id);
                $query = $this->db->get();
                $data = $query->row();
                $record->reason = $data->reason;
                $record->color = $data->color;
            }
            else if($record->blocked_status == 1 && $record->blocked_way == 2){
                $record->reason = '';
                $record->color = '';
                $item = $this->get_manual_block_reason($record->block_text_id);
                if ($item != FALSE){
                    $record->reason = $item['value'];
    ...
    public function get_flagged_users_post($userid){
        $this->db->select("blocked_status, blocked_way, block_text_id");
        $this->db->from("post");
    ...
    public function get_manual_block_reason($block_text_id) {
        $manual_block_reason = $this->common->manual_block_reason;
        foreach($manual_block_reason as $item) {
            if ($item['id'] == $block_text_id) {
                return $item;
            }
        }
        return FALSE;
    }
    public function count_postfilter_list($show_list, $pstatus, $userid=-1) {
        $query = $this->_post_filter_list($show_list, $pstatus, $userid);
        return $result = $this->db->query($query)->num_rows();
    }

    public function manual_blockpost($id, $reasonid) {
        $user_id = $this->session->userdata('user_id');
        $savedata = array('updated_by' => $user_id, 'updated_at' => date('Y-m-d H:i:s'), 'status' => 0,
            'blocked_status' => 1, 'blocked_way' => 2, 'block_text_id' => intval($reasonid) );
        $this->db->where('id', $id);
        $result = $this->db->update($this->table_name, $savedata);
        if ($result) {
            return true;
    ...
    public function manual_unblockpost($id) {
        $user_id = $this->session->userdata('user_id');
        $savedata = array('updated_by' => $user_id, 'updated_at' => date('Y-m-d H:i:s'), 'status' => 1,
            'blocked_status' => 0, 'blocked_way' => 0, 'block_text_id' => 0 );
        $this->db->where('id', $id);
        $result = $this->db->update($this->table_name, $savedata);
        if ($result) {
            return true;
    ...
    #----------Group Post List Pagination End---------#

    #-----------Page Post List Pagination-------------#
    public function count_page_post_list() {
        $query = $this->_page_post_list(NULL);
        return $result = $this->db->query($query)->num_rows();
    }

    public function pagePostList($show_list = 'show_list') {
        $query = $this->_page_post_list($show_list);
        return $result = $this->db->query($query)->result();
    }

    private function _page_post_list($param = NULL) {
        $sql = $sql_custom = array();
        $f_sql = $f_sql_custom = $custom_where = '';
        foreach ($this->column_search as $cmn) {
            if (isset($_POST["search"]["value"]) && $_POST["search"]["value"] != '') {
        ...
        if ($param == 'show_list' && isset($_POST["length"]) && $_POST["length"] != - 1) {
            ...
            $this->db->select("gp.id AS data_item_id, gp.user_id AS data_userid, gp.content AS data_content, gp.show_status AS data_privacy, gp.share_count AS data_sharecount,  gp.sponsered AS data_sponsered, g.name AS data_page_name, gp.created_at AS data_created_at,  gp.updated_at AS data_updated_at, gp.image_id AS data_image, gp.video_id AS data_video, g.tags AS data_tags, g.title AS data_item_type, gp.post_type AS data_category, g.display_image AS data_price, b.first_name AS data_f_name, b.last_name AS data_l_name, b.gender AS data_gender, c.image AS data_userimage, c.thumb_image AS data_thumb_image,gp.status AS status");

                $this->db->from('page_posts gp');
                $this->db->join('pages g', 'g.id = gp.user_id_written_on', 'LEFT');
                $this->db->join('users b', 'b.id = gp.user_id', 'LEFT');
                $this->db->join('users_profile c', 'b.id = c.user_id', 'LEFT');
