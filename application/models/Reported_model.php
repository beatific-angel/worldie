    ...
    class Reported_model extends CI_Model{
        ...
        public function getReportedContent(){
			...
			foreach ($content_types as $key => $value){ 
			    if($key == 5){
                    $query = $this->db->query('SELECT r.id FROM reported_content as r WHERE r.content_type = '.$key.' AND r.report_id IN(SELECT id FROM event_comment)');
		...
		public function getReportedItemDetails($item_type = null, $item_id = null){
			...
			else if($tablname == 'group_post_comment'){
				$this->db->select('a.*,b.first_name,b.last_name');
				$this->db->from("$tablname a");
				$this->db->join('users b', 'a.created_by = b.id');
				$this->db->where('a.id', $item_id);
            ...
            else if($tablname == 'media_video_comment'){
				$this->db->select('a.*,b.first_name,b.last_name');
				$this->db->from("$tablname a");
				$this->db->join('users b', 'a.created_by = b.id');
				$this->db->where('a.id', $item_id);
			...
			else if($tablname == 'Page_Post_Comment'){
				$this->db->select('a.*,b.first_name,b.last_name');
				$this->db->from("$tablname a");
				$this->db->join('users b', 'a.created_by = b.id');
				$this->db->where('a.id', $item_id);
			...
			else if($tablname == 'store_products'){
				$this->db->select('a.*,b.first_name,b.last_name');
				$this->db->from("$tablname a");
				$this->db->join('users b', 'a.created_by = b.id');
				$this->db->where('a.id', $item_id);
			...
			else if($tablname == 'store_product_comment'){
				$this->db->select('a.*,b.first_name,b.last_name');
				$this->db->from("$tablname a");
				$this->db->join('users b', 'a.created_by = b.id');
				$this->db->where('a.id', $item_id);
			...
			else if($tablname == 'wall_arts_comment'){
				$this->db->select('a.*,b.first_name,b.last_name');
				$this->db->from("$tablname a");
				$this->db->join('users b', 'a.created_by = b.id');
				$this->db->where('a.id', $item_id);
			...
			else if($tablname == 'group_comment'){
				$this->db->select('a.*,b.first_name,b.last_name');
				$this->db->from("$tablname a");
				$this->db->join('users b', 'a.created_by = b.id');
				$this->db->where('a.id', $item_id);
		...
		public function getTableName($type = null){
			switch ($type) {
				...
				case "Page_Post_Comment":
				    return "page_post_comment";
				    break;
			    case "Store_Product":
				    return "store_products";
				    break;
			    case "Store_Product_Comment":
				    return "store_product_comment";
				    break;
				default:
					break;
			}
		...
		public function sendReviewNotification($data = null){
			...
			if($data['item_type']=="Post" || $data['item_type']=="Post_Comment"){
				...
				$item_type = 16;
				$title="Your reported content for Product has been reviewed";
		 	}else if($data['item_type']=="Page"){
		 		$this->db->where('id',$item_id);
				$result=$this->db->get('pages')->row();
				$item_id=$result->id;
				$item_type = 17;
				$title="Your reported content for Page has been reviewed";
		 	}
		...
		#---------pagination reported content------------#
	public function count_reported_list() {
        $query = $this->_reported_list(NULL);
        return $result = $this->db->query($query)->num_rows();
    }

    public function reportedList($show_list = 'show_list') {
        $query = $this->_reported_list($show_list);
        return $result = $this->db->query($query)->result();
    }

    private function _reported_list($param = NULL) {
    	$tablename = $this->table_name;
        ...
        if(isset($_POST['content_type']) && !empty($_POST['content_type'])){
          $sql_custom[] = "a.content_type = '".$_POST['content_type']."'";
        }

        if(isset($_POST['content_reason']) && !empty($_POST['content_reason'])){
          $sql_custom[] = 'a.report_reason LIKE '."'%".$_POST["content_reason"]."%'";
        }
        ...
        if ($param == 'show_list' && isset($_POST["length"]) && $_POST["length"] != - 1) {
			...
			$this->db->from("$tablename a");
			$this->db->join('users b', 'a.user_id = b.id');
			$this->db->where('a.isReviewed', 0);
			$this->db->where('(a.content_type = 1 OR a.content_type = 2 OR a.content_type = 3 OR a.content_type = 4 OR a.content_type = 5 OR a.content_type = 6 OR a.content_type = 7 OR a.content_type = 8 OR a.content_type = 9 OR a.content_type = 10 OR a.content_type = 11 OR a.content_type = 12 OR a.content_type = 13 OR a.content_type = 14 OR a.content_type = 15 OR a.content_type = 16 OR a.content_type = 17)');
