...
class Ion_auth_model extends CI_Model
{
	 ...
	 * @var array
	 **/
	public $_cache_user_in_role = array();
	 ...
	 * @author Mathew
	 **/
	public function register($identity, $password, $email, $additional_data = array(), $roles = array())
	{
		...
		elseif ( !$this->config->item('default_role', 'ion_auth') && empty($roles) )
		{
		...
		/*$query = $this->db->get_where($this->tables['roles'],array('name' => $this->config->item('default_role', 'ion_auth')),1)->row();
		...
		// add in roles array if it doesn't exists and stop adding into default role if default role ids are set
	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		$query = $this->db->select($this->identity_column . ', email, id, password, status, role_id, first_name, last_name, gender, last_login')
		            ->where($this->identity_column, $identity)->where('role_id !=', 5)
		                  ->limit(1)
		    			  ->order_by('id', 'desc')
		                  ->get($this->tables['users']);
		if($this->is_max_login_attempts_exceeded($identity))
		{
			...
			$result = $this->hash_password($password);
...
//die('error password');
        ...
        if($recheck!==0)
        {
            ...
            if($last_login+$recheck < time())
            {
                $query = $this->db->select('id')
                    ->where(array($this->identity_column=>$this->session->userdata('identity'),'status'=>'1'))
                    ->limit(1)
                    ->order_by('id', 'desc')
                    ->get($this->tables['users']);
                if ($query->num_rows() === 1)
	...
	*/
	...
	public function users($roles = NULL)
	{
		...
		if (isset($roles))
		{
			...
			// join and then run a where_in against the role ids
			if (isset($roles) && !empty($roles))
			{
				$this->db->distinct();
				$this->db->join(
				    $this->tables['users_roles'],
				    $this->tables['users_roles'].'.'.$this->join['users'].'='.$this->tables['users'].'.id',
				    'inner'
				);
			}

			// verify if role name or role id was used and create and put elements in different arrays
			$role_ids = array();
			$role_names = array();
			foreach($roles as $role)
			{
				if(is_numeric($role)) $role_ids[] = $role;
				else $role_names[] = $role;
			}
			$or_where_in = (!empty($role_ids) && !empty($role_names)) ? 'or_where_in' : 'where_in';
			// if role name was used we do one more join with roles
			if(!empty($role_names))
			{
				$this->db->join($this->tables['roles'], $this->tables['users_roles'] . '.' . $this->join['roles'] . ' = ' . $this->tables['roles'] . '.id', 'inner');
				$this->db->where_in($this->tables['roles'] . '.name', $role_names);
			}
			if(!empty($role_ids))
			{
				$this->db->{$or_where_in}($this->tables['users_roles'].'.'.$this->join['roles'], $role_ids);
			}
		}
	...
	{
		$this->trigger_events('get_users_role');

		// if no id was passed use the current users id
		$id || $id = $this->session->userdata('user_id');

		return $this->db->select($this->tables['users_roles'].'.'.$this->join['roles'].' as id, '.$this->tables['roles'].'.name, '.$this->tables['roles'].'.description')
		                ->where($this->tables['users_roles'].'.'.$this->join['users'], $id)
		                ->join($this->tables['roles'], $this->tables['users_roles'].'.'.$this->join['roles'].'='.$this->tables['roles'].'.id')
	 ...
	 * @author Ben Edmunds
	 **/
	public function add_to_role($role_ids, $user_id=false)
	{
		$this->trigger_events('add_to_role');
		...
		{
			if ($this->db->insert($this->tables['users_roles'], array( $this->join['roles'] => (float)$role_id, $this->join['users'] => (float)$user_id)))
			{
				if (isset($this->_cache_roles[$role_id])) {
					$role_name = $this->_cache_roles[$role_id];
				}
				else {
					$role = $this->role($role_id)->result();
					$role_name = $role[0]->name;
					$this->_cache_roles[$role_id] = $role_name;
				}
				$this->_cache_user_in_role[$user_id][$role_id] = $role_name;
	 ...
	 * @author Ben Edmunds
	 **/
	public function remove_from_role($role_ids=false, $user_id=false)
	{
		$this->trigger_events('remove_from_role');
		...
		if( ! empty($role_ids))
		{
			...
			foreach($role_ids as $role_id)
			{
				$this->db->delete($this->tables['users_roles'], array($this->join['roles'] => (float)$role_id, $this->join['users'] => (float)$user_id));
				if (isset($this->_cache_user_in_role[$user_id]) && isset($this->_cache_user_in_role[$user_id][$role_id]))
				{
					unset($this->_cache_user_in_role[$user_id][$role_id]);
				}
			}
		...
		{
			if ($return = $this->db->delete($this->tables['users_roles'], array($this->join['users'] => (float)$user_id))) {
				$this->_cache_user_in_role[$user_id] = array();
			}
		}
		return $return;
	}
	...
	**/
	public function delete_user($id)
	{
		...
		$this->remove_from_role(NULL, $id);

		// delete user from users table should be placed after remove from role
	...
	public function update_last_login($id)
	{
		...
		$result= $this->db->update($this->tables['users'], array('last_login' => time()), array('id' => $id));
	...
	public function set_session($user)
	{
		...
		$session_data = array(
			...
			'gender'               => $user->gender,
			'role_id'              => $user->role_id, //everyone likes to overwrite id so we'll use user_id
	 ...
	 * @author aditya menon
	*/
	public function create_role($role_name = FALSE, $role_description = '', $additional_data = array())
	{
		// bail if the role name was not passed
		if(!$role_name)
		{
			$this->set_error('role_name_required');
			return FALSE;
		}

		// bail if the role name already exists
		$existing_role = $this->db->get_where($this->tables['roles'], array('name' => $role_name))->num_rows();
		if($existing_role !== 0)
		{
			$this->set_error('role_already_exists');
			return FALSE;
		}
		...
		$this->trigger_events('extra_role_set');

		// insert the new role
		$this->db->insert($this->tables['roles'], $data);
		$role_id = $this->db->insert_id($this->tables['roles'] . '_id_seq');
	 ...
	 * @author aditya menon
	 **/
	public function update_role($role_id = FALSE, $role_name = FALSE, $additional_data = array())
	{
		...
		if (!empty($role_name))
		{
			...
			// bail if the role name already exists
			$existing_role = $this->db->get_where($this->tables['roles'], array('name' => $role_name))->row();
			if(isset($existing_role->id) && $existing_role->id != $role_id)
			{
				$this->set_error('role_already_exists');
				return FALSE;
			}
        ...
        $role = $this->db->get_where($this->tables['roles'], array('id' => $role_id))->row();
        if($this->config->item('admin_role', 'ion_auth') === $role->name && $role_name !== $role->name)
        {
            $this->set_error('role_name_admin_not_alter');
            return FALSE;
        }
		...
		$this->db->update($this->tables['roles'], $data, array('id' => $role_id));
	...
	* @author aditya menon
	**/
	public function delete_role($role_id = FALSE)
	{
		...
		$role = $this->role($role_id)->row();
		if($role->name == $this->config->item('admin_role', 'ion_auth'))
		{
			$this->trigger_events(array('post_delete_role', 'post_delete_role_notallowed'));
			$this->set_error('role_delete_notallowed');
			return FALSE;
		}

		$this->trigger_events('pre_delete_role');

		$this->db->trans_begin();

		// remove all users from this role
		$this->db->delete($this->tables['users_roles'], array($this->join['roles'] => $role_id));
		// remove the role itself
		$this->db->delete($this->tables['roles'], array('id' => $role_id));

		if ($this->db->trans_status() === FALSE)
		{
			$this->db->trans_rollback();
			$this->trigger_events(array('post_delete_role', 'post_delete_role_unsuccessful'));
		...
		$this->trigger_events(array('post_delete_role', 'post_delete_role_successful'));
