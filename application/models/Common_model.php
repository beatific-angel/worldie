<?php 
	...
	class Common_model extends CI_Model {
		...
		public function __construct(){
			...
			$this->block_colors = array('red', 'orange', 'yellow');
			$this->manual_block_reason = array(
				array("id"=>1, "value"=> "motherfuck", "color"=> "yellow"),
				array("id"=>2, "value"=> "cyberfuck", "color"=> "orange"),
				array("id"=>3, "value"=> "son of a bitch", "color"=> "red"),
			);
			$this->blockfile_color = 'red';
			$this->blockfile_reason = 'word in list of bad and swear words';
			$this->blockcsvfile_suffix = array('');
        ...
        public function count_messages_list(){

			return $this->db->count_all_results('users_chat_msgs');
	...
	public function getTabPermission($role_id,$controller_name){

		$this->db->select('b.*');

		$this->db->from('tbl_sidebar a'); 

		$this->db->join('tbl_role_permission b','a.id = b.tab_id','inner');

		$this->db->where('b.role_id', $role_id);

		$this->db->where('a.controller_name', $controller_name);
	...
	/* get data by table column & value */

	public function getRowDetails($table,$where){
	...
	public function updateData($table,$post,$where){
	...
	public function getParentTabAsPerRole($role_id){
		...
		$this->db->where('b.view_status', 1);

		$this->db->where('a.parent_id', 0);

		$this->db->where('a.status', 1);

		$this->db->order_by('a.tab_number', 'ASC');
	...
	public function getData($tbl_name , $where_array = NULL , $fetch_type = 'multi', $order_by = NULL , $order_by_clm = NULL , $limit = NULL , $offset = NULL , $check_query = NULL,$select = NULL){
		...
		if($order_by != NULL && $order_by_clm != NULL){
