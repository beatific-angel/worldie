...
if( ! function_exists('generateCommentHtml')){
	function generateCommentHtml($comments, $element_id, $content_type, $comm_count, $showLimit = 4, $bulk_delete = "0"){
		$CI =& get_instance();
		$base_url_front = $CI->config->item('base_url_front'); 
		...
		if(!empty($comments)){ 
			...
			foreach($comments as $comment){
			    $checkHtml = '';
    			if($bulk_delete == '1'){
    			    $checkHtml = '<input type="checkbox" style="float: left;left: 26px;position: absolute;" class="comments" value="'.$type.'-'.$comment->comment_id.'" data-ip="'.$comment->ip_address.'" data-user_id="'.$comment->cu_userid.'">';
    			}
				$commentCounter++;
				...
				<div class="right-content">
					...
					<div class="actions">';
					    $typee = "'".$type."'";
						$html .='<a href="javascript:;"><img border="0" alt="img" src="'.$base_url_front.'assets/images/cancel_icon_new.png" onclick="deleteComment('.$comment->comment_id.', '.$element_id.', '.$typee.', '.$content_type_id.', '.$comment->parent_id.', this);" title="Delete"></a>';
					$html .='</div>';
				
					if(count($comment->reply) ==  0){
					    $typee = "'".$type."'";
						$html .='<div class="view-hide '.$type.'_show_hide_reply_div_button_'.$comment->comment_id.'" style="display:none;"><a href="javascript:;" onclick="showHideReplyArea('.$comment->comment_id.', '.$typee.');">View all replies</a></div>';
					}else{
					    $typee = "'".$type."'";
						$html .='<div class="view-hide '.$type.'_show_hide_reply_div_button_'.$comment->comment_id.'"><a href="javascript:;" onclick="showHideReplyArea('.$comment->comment_id.', '.$typee.');">View all '.$comment->reply_count.' replies</a></div>';
					}
					$html .='</div>';
					...
					if(count($comment->reply) > 0){ 
						foreach($comment->reply as $reply){
							$html .= getCommentReplyHtml($reply, $element_id, $type, 2,$bulk_delete);
						}}
						$html .= '</div></ul></li></div>';
					} }
					$html .='</div>
...
if( ! function_exists('getCommentReplyHtml')){
	function getCommentReplyHtml($reply, $element_id, $type, $content_type_id, $bulk_delete){
		...
		$checkHtml = '';
		if($bulk_delete == '1'){
		    $checkHtml = '<input type="checkbox" style="float: left;left: 26px;position: absolute;" class="comments"  value="'.$type.'-'.$reply->comment_id.'" data-ip="'.$reply->ip_address.'" data-user_id="'.$reply->cu_userid.'">';
		}
		$html .= '<ul class="'.$type.'_comment_reply_div_'.$reply->comment_id.' my_comment"><li>'.$checkHtml.'
				<section>
					<div class="dp-image">';
						$reply_user_image = '';
						...
						$html .='<div class="actions">';
						        $typee = "'".$type."'";
								$html .='<a href="javascript:;"><img border="0" alt="img" src="'.$base_url_front.'assets/images/cancel_icon_new.png" onclick="deleteComment('.$reply->comment_id.', '.$element_id.', '.$typee.', '.$content_type_id.', '.$reply->parent_id.',this);" title="Delete"></a>
							</div>';
...
if( ! function_exists('getContentTypeId')){
	function getContentTypeId($content_type){
        $CI =& get_instance();
		$CI->config->load('user_permission', TRUE);
		$c_type  = $CI->config->item('content_type', 'user_permission');
		return $c_type[$content_type];
	}
}

if( ! function_exists('redirectWithMsgSuccess')){
function redirectWithMsgSuccess($msg,$url,$mess_variable='success_val', $is_baseurl=FALSE){
	    $CI =& get_instance();
		$CI->session->set_flashdata($mess_variable, $msg);
		if ($is_baseurl){
			redirect($url);
		}else{
...
if( ! function_exists('redirectWithMsgFailed')){
	 function redirectWithMsgFailed($msg,$url,$mess_variable='error_val', $is_baseurl=FALSE){
	 	$CI =& get_instance();
		$CI->session->set_flashdata($mess_variable,$msg);
		if ($is_baseurl){
			redirect($url);
